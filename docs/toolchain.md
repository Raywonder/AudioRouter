...
The current driver source is in `src\ASIOA.Driver`. It exposes the 68 input/68 output ASIO surface, COM registration hooks, ASIO registry registration, channel names, sample-rate/buffer metadata, and safe silence until the engine/shared-memory transport is connected. VST3 builds will be added once the C++ toolchain and SDK licensing path are fully repaired.
